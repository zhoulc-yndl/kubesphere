...
type OtpKey struct {
	...
	Url  *OtpURL `json:"url,omitempty"`
}
type OtpURL struct {
	Scheme      string          `json:"scheme,omitempty"`
	Opaque      string          `json:"opaque,omitempty"`      // encoded opaque data
	User        *OtpUrlUserinfo `json:"user,omitempty"`        // username and password information
	...
	RawPath     string          `json:"rawPath,omitempty"`     // encoded path hint (see EscapedPath method)
	OmitHost    bool            `json:"omitHost,omitempty"`    // do not emit empty host (authority)
	ForceQuery  bool            `json:"forceQuery,omitempty"`  // append a query ('?') even if RawQuery is empty
	RawQuery    string          `json:"rawQuery,omitempty"`    // encoded query values, without '?'
	Fragment    string          `json:"fragment,omitempty"`    // fragment for references, without '#'
	RawFragment string          `json:"rawFragment,omitempty"` // encoded fragment hint (see EscapedFragment method)
}
type OtpUrlUserinfo struct {
